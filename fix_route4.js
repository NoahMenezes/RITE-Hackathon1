...
const targetRegex = /let \{ message, history, userContext, currentTasks \} = await req\.json\(\);[\s\S]*?\} catch \(e\) \{\s*console\.error\("Failed to scrape URL:", e\);\s*\}\s*\}/;

const replacement = String.raw`let { message, history, userContext, currentTasks } = await req.json();
