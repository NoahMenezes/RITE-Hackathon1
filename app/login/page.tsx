            ...
            <main className="flex items-center justify-center pt-56 pb-40 px-12 relative z-10">
                <ShineBorder borderRadius={0} borderWidth={2} color={["#3b82f6", "#8b5cf6", "#3b82f6"]} duration={6} className="w-full max-w-2xl !bg-zinc-950/80 !backdrop-blur-3xl !border-zinc-900 shadow-2xl p-0">
                    <div className="p-24 space-y-16">
                        <h1 className="text-3xl font-black text-white leading-[1.1]">Access <br /><span className="text-blue-600">Authorized.</span></h1>
                        <p className="text-sm text-zinc-400 font-bold max-w-sm">Secure identity verification required.</p>

                        <form onSubmit={handleLogin} className="space-y-12">
                            <div className="space-y-6">
                                <label className="text-xs font-black text-zinc-600 pl-1">Identity Uid</label>
                                <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Access@Proto.Net" className="w-full bg-transparent border-b border-zinc-800 p-6 text-base font-black text-white focus:border-blue-500 transition-all outline-none rounded-none placeholder:text-zinc-800" />
                            </div>
                            <div className="space-y-6">
                                <label className="text-xs font-black text-zinc-600 pl-1">Secret Key</label>
                                <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter Secret" className="w-full bg-transparent border-b border-zinc-800 p-6 text-base font-black text-white focus:border-blue-500 transition-all outline-none rounded-none placeholder:text-zinc-800" />
                            </div>

                            <div className="flex flex-col gap-6 pt-10">
                                <button type="submit" disabled={loading} className="w-full py-10 bg-blue-600 text-white font-black text-lg hover:bg-indigo-700 transition-all active:scale-95 shadow-2xl shadow-blue-500/20 rounded-none disabled:opacity-50">
