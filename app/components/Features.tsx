                ...
                {features.map((feature, i) => (
                    <ShineBorder key={i} borderRadius={0} borderWidth={1} color={["#3b82f6", "#8b5cf6", "#3b82f6"]} duration={6} className="w-full !p-0 !bg-zinc-950/40 !backdrop-blur-3xl !border-zinc-900 group transition-all hover:!bg-zinc-900/60 min-h-[500px]">
                        <div className="p-20 h-full flex flex-col justify-center relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-10 text-zinc-900 font-extrabold text-6xl group-hover:text-blue-500/20 transition-all uppercase tracking-widest select-none opacity-40">M0{i + 1}</div>
                            <div className="space-y-12">
                                <h3 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter leading-tight group-hover:text-blue-500 transition-colors border-l-8 border-blue-600 pl-10 underline decoration-blue-600 underline-offset-8 decoration-4">{feature.title}</h3>
                                <p className="text-zinc-400 font-bold text-xl md:text-2xl leading-relaxed group-hover:text-zinc-100 transition-colors uppercase tracking-tight max-w-2xl">{feature.description}</p>
                            </div>
                        </div>
                    </ShineBorder>
                ))}
